 ...
 */
DSDEF int ds_generate_Keys(size_t keysize, String_Builder *privateKey, String_Builder *publicKey);
 ...
 * @return 0 if no error occured, 1 otherwise
 */
DSDEF int ds_signature(String_View secret, String_View message, String_Builder *sign);
 ...
 */
DSDEF int ds_isValid(String_View publicKey, String_View message, String_View sign);
 ...
 * 
 * @note all strings view returned by functions of this library must be deallocated by hand
 * 
 * @return 0 if no error occured, 1 otherwise
 */
