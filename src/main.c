...
void execute_command(String_View sv_command, String_View sv_input) {
  ...
  if (sv_eq(sv_command, sv_from_cstr("createUser"))) {
    String_Builder sb_private_path =
        sv_to_sb_null(sv_chop_by_spaces(&sv_input));
    ...
    String_Builder sb_file_to_sign_path =
        sv_to_sb_null(sv_chop_by_spaces(&sv_input));
    ...
    control(ds_signature(sb_to_sv(sb_private_key), sb_to_sv(sb_file_to_sign),
                         &sb_sign));
    ...
    String_Builder sb_signature_path =
        sv_to_sb_null(sv_chop_by_spaces(&sv_input));
    ...
    String_Builder sb_sign = {0};
    control(
        ds_signature(sb_to_sv(sb_private_key), sb_to_sv(sb_coin), &sb_sign));
    ...
    String_Builder sb_public_key_path =
        sv_to_sb_null(sv_chop_by_spaces(&sv_input));
    ...
    String_Builder sb_message_path =
        sv_to_sb_null(sv_chop_by_spaces(&sv_input));
    ...
    int value = ds_isValid(sb_to_sv(sb_public_key), sb_to_sv(sb_message),
                           sb_to_sv(sb_signature));
